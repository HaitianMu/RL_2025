...
public partial class HumanControl : MonoBehaviour
{
    private void LeaderUpdate_Clam()
    {
        if (myTargetDoor is null)
        {
            ...
            {//print($"距离门对面还有{distanceRemain}米，扫描沿路是否有合适的领导者");
                List<GameObject> leaderCandidates = GetCandidate_Clam(new List<string> { "Human", "Robot" }, 360, visionLimit).Item1;
            ...
            (List<GameObject> doorCandidates, List<Vector3> unknownDirections) = GetCandidate_Clam(new List<string> { "Door", "Exit" }, 360, visionLimit);
    ...
    private void FollowerUpdate_Clam()
    {
        ...
        List<GameObject> exitList = GetCandidate_Clam(new List<string> { "Exit" }, 360, visionLimit).Item1;
